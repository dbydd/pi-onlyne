...
export default function onlyne(pi: ExtensionAPI) {
	...
	pi.registerTool(defineTool({ name: "onlyne_send", label: "Onlyne send", description: "Send plain text to one Onlyne channel conversation.", parameters: Type.Object({ channelId: Type.String(), conversationId: Type.String(), text: Type.String() }), executionMode: "parallel", async execute(_id, params) { if (!state.workspace) throw new Error("onlyne workspace not found"); const res = await sendWithRetry(state.workspace.socketPath, params, params.text, currentConfig().outbound.retry.attempts); return textResult(JSON.stringify(res), res); } }));
	pi.registerTool(defineTool({ name: "onlyne_broadcast", label: "Onlyne broadcast", description: "Send plain text to many Onlyne channel conversations concurrently.", parameters: Type.Object({ targets: Type.Array(Type.Object({ channelId: Type.String(), conversationId: Type.String() })), text: Type.String() }), executionMode: "parallel", async execute(_id, params) { if (!state.workspace) throw new Error("onlyne workspace not found"); const cfg = currentConfig(); const results = await broadcast(state.workspace.socketPath, params.targets as SendTarget[], params.text, cfg.outbound.retry.attempts, cfg.outbound.retry.concurrency); return textResult(JSON.stringify({ ok: results.every((r) => r.ok), results }), results); } }));
